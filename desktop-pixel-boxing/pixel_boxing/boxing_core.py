...
ROUND_LIMIT = 3
ROUND_SECONDS = 45.0
DODGE_STAMINA_COST = 17.0
BACKSTEP_STAMINA_COST = 31.0
STAMINA_RECOVERY_PER_SEC = 8.5
EXHAUSTION_DURATION = 1.1
EXHAUSTION_EXPOSED = 0.28

WHIFF_RECOVERY = 0.22
EXPOSED_DMG_MULT = 1.4
BODY_HOOK_COUNTER_MULT = 1.2
STAGGER_LOCK = 0.16
STALE_THRESHOLD = 2
STALE_DECAY = 0.85
STALE_FLOOR = 0.5
...
ATTACK_SPECS = {
    "jab": AttackSpec(0.24, 0.08, 0.15, 6, 78.0, 26.0, 13, "head", 5.0),
    "cross": AttackSpec(0.34, 0.14, 0.24, 11, 60.0, 20.0, 18, "head", 5.4),
    "left_body": AttackSpec(0.32, 0.12, 0.22, 9, 54.0, 40.0, 16, "body", 5.2),
    "right_body": AttackSpec(0.35, 0.14, 0.24, 10, 56.0, 38.0, 17, "body", 5.4),
    "left_hook": AttackSpec(0.36, 0.16, 0.26, 11, 50.0, 56.0, 18, "head", 5.6),
    "right_hook": AttackSpec(0.38, 0.17, 0.28, 12, 52.0, 50.0, 19, "head", 5.8),
    "left_uppercut": AttackSpec(0.40, 0.18, 0.30, 13, 44.0, 26.0, 20, "head", 5.4),
    "right_uppercut": AttackSpec(0.42, 0.19, 0.31, 14, 46.0, 24.0, 21, "head", 5.8),
}
...
def try_start_attack(actor: object, name: str) -> AttackStartResult:
    ...
    if getattr(actor, "exhausted", 0.0) > 0:
        return AttackStartResult(False, reason="exhausted")
    ...
    spend_stamina(actor, spec.stamina_cost)
    return AttackStartResult(True, stale_stage=stale_stage)


def spend_stamina(actor: object, amount: float) -> bool:
    actor.stamina = clamp(actor.stamina - amount, 0.0, actor.max_stamina)
    if actor.stamina > 1e-9:
        return False
    actor.exhausted = max(getattr(actor, "exhausted", 0.0), EXHAUSTION_DURATION)
    actor.exposed = max(getattr(actor, "exposed", 0.0), EXHAUSTION_EXPOSED)
    return True


def counter_damage(action: str, base_damage: int, punished_whiff: bool) -> int:
    if not punished_whiff:
        return base_damage
    damage = int(round(base_damage * EXPOSED_DMG_MULT))
    if ATTACK_FAMILY.get(action) in {"body", "hook"}:
        damage = int(round(damage * BODY_HOOK_COUNTER_MULT))
    return damage
...
class PlayerHabitMemory:
    attack_counts: dict[str, float] = field(
        default_factory=lambda: {action: 0.0 for action in ATTACK_ACTIONS}
    )
    guard_samples: float = 0.0
    duck_left_samples: float = 0.0
    duck_right_samples: float = 0.0
    backstep_samples: float = 0.0
    pressure_samples: float = 0.0
    retreat_samples: float = 0.0
...
@dataclass(frozen=True)
class PlayerHabitSnapshot:
    favorite_attack: str
    jab_ratio: float
    straight_ratio: float
    body_ratio: float
    hook_ratio: float
    uppercut_ratio: float
    guard_ratio: float
    duck_left_ratio: float
    duck_right_ratio: float
    backstep_ratio: float
...
def note_player_attack(memory: PlayerHabitMemory, action: str) -> None:
    if action not in ATTACK_ACTIONS:
        return
    memory.attack_counts[action] += 1.0
    if memory.recent_attack == action:
        memory.repeat_streak += 1
    else:
        memory.recent_attack = action
        memory.repeat_streak = 1


def sample_player_tendency(
    ...
    *,
    ...
    sample = max(dt * 8.0, 0.0)
    memory.total_samples += sample
    if guarding:
        memory.guard_samples += sample
    if duck_dir == "q":
        memory.duck_left_samples += sample
    elif duck_dir == "e":
        memory.duck_right_samples += sample
    if backstep:
        memory.backstep_samples += sample
    if forward_axis > 0:
        memory.pressure_samples += sample * abs(forward_axis)
    elif forward_axis < 0:
        memory.retreat_samples += sample * abs(forward_axis)


def snapshot_player_habits(memory: PlayerHabitMemory) -> PlayerHabitSnapshot:
    total_attacks = sum(memory.attack_counts.values()) or 1.0
    sample_total = max(memory.total_samples, 1.0)
    favorite_attack = max(memory.attack_counts, key=memory.attack_counts.get, default="jab")
    return PlayerHabitSnapshot(
        favorite_attack=favorite_attack,
        jab_ratio=memory.attack_counts["jab"] / total_attacks,
        straight_ratio=(memory.attack_counts["jab"] + memory.attack_counts["cross"]) / total_attacks,
        body_ratio=(memory.attack_counts["left_body"] + memory.attack_counts["right_body"]) / total_attacks,
        hook_ratio=(memory.attack_counts["left_hook"] + memory.attack_counts["right_hook"]) / total_attacks,
        uppercut_ratio=(memory.attack_counts["left_uppercut"] + memory.attack_counts["right_uppercut"]) / total_attacks,
        guard_ratio=memory.guard_samples / sample_total,
        duck_left_ratio=memory.duck_left_samples / sample_total,
        duck_right_ratio=memory.duck_right_samples / sample_total,
        backstep_ratio=memory.backstep_samples / sample_total,
        pressure_ratio=memory.pressure_samples / sample_total,
        retreat_ratio=memory.retreat_samples / sample_total,
        repeat_streak=memory.repeat_streak,
        sample_strength=min(1.0, sample_total / 18.0),
    )


def adaptation_level(snapshot: PlayerHabitSnapshot, round_number: int) -> float:
    round_factor = 0.16 * max(0, round_number - 1)
    return clamp(0.18 + round_factor + 0.58 * snapshot.sample_strength, 0.18, 1.0)


def adaptive_enemy_weights(
    snapshot: PlayerHabitSnapshot,
    distance: float,
    round_number: int,
    *,
    player_exposed: bool,
) -> dict[str, float]:
    adapt = adaptation_level(snapshot, round_number)
    weights = {
        "jab": 0.1,
        "cross": 0.1,
        "left_body": 0.08,
...
class HeadlessCombatSimulator:
    ...
    def valid_actions(self, actor: HeadlessFighter) -> dict[str, bool]:
        free = actor.action_t >= actor.action_dur and actor.stagger <= 0 and actor.exhausted <= 0
        return {
            action: (
                action == "wait"
                or free
                and (
                    action in ATTACK_SPECS
                    and actor.stamina >= ATTACK_SPECS[action].stamina_cost
                    or action == "backstep"
                    and actor.stamina >= BACKSTEP_STAMINA_COST
                    or action not in ATTACK_SPECS
                    and action != "backstep"
                )
            )
    ...
    def _apply_decision(
        ...
        if action == "approach":
            ...
            spend_stamina(actor, BACKSTEP_STAMINA_COST)
    ...
    def _advance_actor(
        ...
        dt = self.decision_dt
        actor.stamina = clamp(actor.stamina + dt * STAMINA_RECOVERY_PER_SEC, 0.0, actor.max_stamina)
